...
public class BillPrint {
    public BillPrint(String link) throws FileNotFoundException, UnsupportedEncodingException, JRException {
        InputStreamReader file = new InputStreamReader(new FileInputStream(link), "UTF-8"); //input檔案，檔案編碼為UTF-8
        List<BillBean> beans = new CsvToBeanBuilder<BillBean>(file) //將CSV 儲存進beans，以供後續使用
                .withType(BillBean.class)
                .withSkipLines(1)
                .build()
                .parse();

        JRDataSource dataSource = new JRBeanCollectionDataSource(beans);
        JasperDesign design = JRXmlLoader.load("layout/bill.jrxml"); // 版面位置
